...
sentence <- tolower("On the first day I overslept and missed classes")
...
CountOccurances <- function(letter, letter.vector) {
  occurances <- length(letter.vector[letter.vector == letter])
  return(occurances)
}

# Call your `CountOccurances()` function to see how many times the letter 'e' is
# in your sentence.
e.occurance <- CountOccurances("e", letters)
...
all.occurances <- as.list(sapply(unique.letters, CountOccurances, letters))
